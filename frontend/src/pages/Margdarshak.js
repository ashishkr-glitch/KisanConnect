...
function Margdarshak() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Helper: POST with exponential backoff on 429s and respect Retry-After header
      const postWithBackoff = async (url, body, opts = {}, maxRetries = 5) => {
        let attempt = 0;
        while (true) {
          try {
            ...
            // If not a rate-limit, or retries exhausted, rethrow
            if (status !== 429 || attempt >= maxRetries) throw err;
            // Try to read Retry-After header (seconds). Axios headers are lowercase in browsers.
            const raHeader = err.response?.headers?.['retry-after'];
            const ra = raHeader ? parseInt(raHeader, 10) : NaN;
            const backoff = !isNaN(ra) && ra > 0 ? ra * 1000 : Math.min(1000 * 2 ** attempt, 16000);
            console.warn(`[Margdarshak] 429 received — retrying in ${backoff}ms (attempt ${attempt + 1})`, { status, raHeader });
      ...
      const resp = await postWithBackoff(proxyUrl, payload, { headers }, 5);
      ...
      console.error("[Margdarshak] Error calling Gemini API:", {
        message: err?.message,
